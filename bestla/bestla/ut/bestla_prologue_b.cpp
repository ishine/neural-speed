...
class UT_BlockQunatize_INT8 {
 public:
  ...
  void ut(int n, int k, int blocksize, bool asym = false) {
    ...
    using PrologueB = prologue_b::gemm::WeightPack<gemm::SCoreRowNAvx512f<48, 8>, RuntimeISA>;
    PrologueB kernel;
    // auto ptr = kernel.createStorage(n, k, blocksize, BTLA_DTYPE::S8, bestla_dtype<float>, bestla_dtype<float>, asym);
    auto ptr = kernel.createStorage(n, k);
...
//     auto packedW = ProWei.createStorage(n, k, blocksize, qtype, bestla_dtype<float>, bestla_dtype<utils::bf16>,
//     asym); avector<int8_t> buf0(packedW.mSize), buf1(packedW.mSize); packedW.assign(buf0.data());
...
//     PrologueB ProWei;
//     auto packedW = ProWei.createStorage(n, k, blocksize, qtype, bestla_dtype<float>, bestla_dtype<utils::bf16>,
//     asym); ProWei.enableShuffle(&packedW); avector<int> groupindices(k, 0); auto groupsize = utils::updiv(k,
//     blocksize); avector<int> reflut(k, 0); for (size_t i = 0; i < k; i++) {
...
//     false); ut_int<sAVX2, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, 32, BTLA_DTYPE::S8, BTLA_DTYPE::F32,
//     false); ut_int<sAVX2, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, 128, BTLA_DTYPE::S8,
//     BTLA_DTYPE::F32, false); ut_int<sAVX2, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, -1, BTLA_DTYPE::S8,
//     BTLA_DTYPE::F32, false);

//     CheckISA(AVX512F);
//     ut_int<sAVX512F, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, 32, BTLA_DTYPE::S8, BTLA_DTYPE::BF16,
//                                                              false);
//     ut_int<sAVX512F, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, 32, BTLA_DTYPE::S8, BTLA_DTYPE::F32,
//     false); ut_int<sAVX512F, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, 128, BTLA_DTYPE::S8,
//     BTLA_DTYPE::F32,
//                                                              false);
//     ut_int<sAVX512F, prologue_b::gemm::WeightKBlockNInteger>(2, 4096, 4096, -1, BTLA_DTYPE::S8, BTLA_DTYPE::F32,
//     false);
//   }
...
//   void benchmark(int m, int n, int k, int blocksize, int batch, float* A, float* B, float* C, float timems, int
//   threads,
//                  BTLA_DTYPE qtype) {
//     LOG_T log;
...
//       ut_newkblock<gemm::ICoreRowNAvx512vnniKBlock<48, 4>>(2, 4096, 4096, 32, BTLA_DTYPE::S4_CLIP,
//       BTLA_DTYPE::DQ8_BNB);
//     }
...
//         {matAu8.data(), k},
//         {&packedw},
//         {packedw.template SPtr<int8_t>(), packedw.SDtype(), packedw.CStep(), scaleAf32.data(), kblks, nullptr,
//         nullptr,
...
//     WType packedw(0);
//     if constexpr (std::is_same_v<Wei<GemmCore_T, ISA>, prologue_b::gemm::WeightKBlockNInteger<GemmCore_T, ISA>>) {
//       packedw = launcher.mProB.createStorage(n, k, blocksize, qtype, bestla_dtype<Scale_T>, bestla_dtype<float>,
//       false);
//     } else if constexpr (std::is_same_v<Wei<GemmCore_T, ISA>, prologue_b::gemm::WeightKBlockNFloat<GemmCore_T, ISA>>)
//     {
...
//       }
//     }
//     launcher.mProB.packNbitsWeightQ4(n, k, isasym, (uint8_t*)matBs4.data(), k, scalesB.data(),
//     (uint8_t*)zpBs4.data(),
